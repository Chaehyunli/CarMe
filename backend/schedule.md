...
완료한 항목은 `- [x]`로 바꾼다. 초기 DB는 [ERD](../docs/erd_및_데이터사전.md)의 11개 테이블만 구현한다.
...
- [ ] `conversation`, `message`, `citation` 모델 작성
...
- [ ] `POST /conversations`, 차량별 이력, 상세 API 구현
- [ ] 빈 채팅방에서 현재 적용 매뉴얼 목록과 download URL API 제공
- [ ] 질문 저장·RAG 실행·상태별 응답 API 구현
...
- [ ] PDF direct upload용 1회성 PUT presigned URL과 완료 검증 API 구현
...
- [ ] worker/CLI로 `UPLOADED → INDEXING → READY/FAILED` 상태 전이
- [ ] citation/매뉴얼 download URL API 구현
...
- [ ] 적재 뒤 수동/스크립트 평가 통과 후에만 관리자가 차량을 공개
