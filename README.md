...
현재 API는 `/api/v1/health`와 `GET /api/v1/vehicle-catalog`을 제공합니다. 차량 카탈로그는 공개 상태이고 기본 매뉴얼이 준비된 모델·연식만 반환합니다. 구현 일정은 [개발 일정](docs/개발_일정_2026-09-15.md), 세부 체크리스트는 [backend/schedule.md](backend/schedule.md), [frontend/schedule.md](frontend/schedule.md), [db/schedule.md](db/schedule.md)를 따릅니다.
