...
CarMe는 선택한 차량에 연결된 `READY` PDF 매뉴얼만 답변의 근거로 사용한다. PDF에 없는 인포테인먼트 세부 절차는 관리자가 검증해 저장한 현대 공식 웹 매뉴얼 **링크**로 안내할 뿐, 접근 제한된 웹 페이지를 자동 수집하거나 인용하지 않는다. 검색어 일부가 겹친다는 이유만으로 관계없는 문단을 답변에 쓰면 안 된다.
...
flowchart TD
    ...
    E -->|PDF 직접 근거 없음·웹 전용 인포테인먼트| W[저장된 현대 공식 웹 매뉴얼 링크 안내]
    W --> I
...
| Retriever | 현재 차량의 준비된 PDF에서 키워드·벡터 후보를 결합해 수집 | 다른 차량·보관 문서·일반 웹 검색 |
...
### 5.3 PDF 밖의 공식 웹 매뉴얼 안내

현대 사이트의 인포테인먼트 매뉴얼은 차량별 웹 화면으로만 제공될 수 있다. 따라서 PDF가 세부 조작을 위임했거나 PDF 근거가 부족할 때, `vehicle_catalog.official_manual_url`에 저장된 차량별 공식 시작 URL을 새 탭 링크로 제공한다.

1. 선택 차량의 `READY` PDF 매뉴얼로 먼저 검색·인용한다.
2. PDF 직접 근거가 없고 내비게이션/블루투스 등 웹 전용 범주면 저장한 공식 URL을 안내한다.
3. 링크 안내는 답변 근거나 citation이 아니며, 웹 페이지 본문을 사실처럼 요약하지 않는다.

Tavily는 이 접근 제한 사이트의 정확한 본문 수집을 보장하지 않으므로 답변 파이프라인의 필수 도구가 아니다. 관리자가 Hyundai Owners Manual에서 확인한 차량별 URL을 직접 등록·수정한다.

웹 링크가 제공된 답변은 UI에서 `현대 공식 웹 매뉴얼 열기`로 표시한다. PDF 근거는 기존처럼 페이지 번호와 인용문을 표시한다.

후보 생성은 정확 일치·동의어·문단 역할 점수와 pgvector cosine 후보를 결합한다. 벡터 서비스가 일시적으로 실패해도 키워드 검색은 계속하되, 벡터 단독 후보는 Evidence Gate의 직접 근거 조건을 통과할 수 없다.
